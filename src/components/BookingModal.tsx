...
export default function BookingModal({
    ...
    // Lock background scroll when modal is open (prevents page scroll fighting modal scroll)
    useEffect(() => {
        if (!open) return;
    ...
    // NOTE: overlay must NOT block scrolling; we implement a scroll container layer
    const overlayBackdrop = "fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm";
    const modalShell =
        "relative w-full max-w-[980px] rounded-3xl border border-white/18 bg-black/55 text-white shadow-[0_30px_110px_rgba(0,0,0,.65)] overflow-hidden";
    const glassInner =
    ...
    return (
        <AnimatePresence>
            {open ? (
                ...
                >
                    {/* Backdrop */}
                    <div className={overlayBackdrop} onClick={close} />

                    {/* Scroll layer (THIS fixes desktop cropping) */}
                    <div className="absolute inset-0 overflow-y-auto">
                        <div className="min-h-full w-full px-4 py-6 md:py-10 flex items-start md:items-center justify-center">
                            <motion.div
                                ...
                                <div className={cx("relative", glassInner)}>
                                    ...
                                    <div className="sticky top-0 z-20 border-b border-white/10 bg-black/30 backdrop-blur-xl">
                                        <div className="flex items-center justify-between gap-4 p-4 md:p-6">
                                            ...
                                            <button
                                                onClick={close}
                                                className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm hover:border-white/30"
                                            >
                                                Close
                                            </button>
                                        </div>
                                    ...
                                    <div className="max-h-[calc(100vh-140px)] overflow-y-auto p-4 md:p-6">
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="mt-4 grid gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <input
                                                                    value={lastRideDate}
                                                                    onChange={(e) => setLastRideDate(e.target.value)}
                                                                    className={field}
                                                            ...
                                                            <div>
                                                                ...
                                                                <input
                                                                    value={lastRideCity}
                                                                    onChange={(e) => setLastRideCity(e.target.value)}
                                                                    className={field}
